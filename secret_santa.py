...
def pull_names(secret_santa_config_file):
    ...
    while restart:
        invalid_matches = [
            sorted(match) for match in secret_santa_config_file.invalid_matches
        ]
        ...
        for gifter in gifter_names:
            ...
            while check_pairing:
                ...
                if (
                    gifter != receiver
                    and sorted([gifter, receiver]) not in invalid_matches
                ):
                ...
                ):  # if only person left has themselves to pick
                    # Start over
                    restart = True
                    check_pairing = False
                elif (len(receiver_names) == 2) and (
                    sorted(receiver_names) in invalid_matches
                ):
    ...
    # Check pairs
    check_pairs = Counter(
        [j for i in [list(pair) for pair in pairs] for j in i]
    ).values()
    pair_warning = any([False if i <= 2 else True for i in check_pairs])
    multiple_names_warning1 = sorted(secret_santa_config.names.keys()) != sorted(
        [i[0] for i in pairs]
    )
    multiple_names_warning2 = sorted(secret_santa_config.names.keys()) != sorted(
        [i[1] for i in pairs]
    )
    if pair_warning:
        print("Repick names!!")
        ...
        # Send emails
        subject = "The 2024 Virtual Holiday Cookie Bake-off!"

        for pair in pairs:
            gifter = pair[0]
            receiver = pair[1]

            file = open("cookie_bakeoff_msg.html", "r")
            msg = file.read()
            msg = msg.format(gifter, receiver)

            gifter_email = secret_santa_config_file.names[gifter]
            send_email(
                from_=secret_santa_config_file.email["username"],
                to_=gifter_email,
